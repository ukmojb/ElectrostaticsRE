        ...
        if (world.isRemote) {
            if (player.motionX != 0 || player.motionZ != 0) {
                if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem().equals(ModItems.rubberBoots)) {
                    Electrostatics.INSTANCE.sendToServer(new ServerMotionPacket((int)player.motionX, (int)player.motionY, false));
                }
                if (player.getItemStackFromSlot(EntityEquipmentSlot.FEET).getItem().equals(ModItems.glassBoots)) {
                    Electrostatics.INSTANCE.sendToServer(new ServerMotionPacket((int)player.motionX, (int)player.motionY, true));
                }
            }
        }
    }
